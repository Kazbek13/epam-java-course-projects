 ...
 * @author Andrew
 */
public class CreateAirportInstanceCommand implements Command {

    static final Logger logger = LogManager.getLogger(CreateFlightInstanceCommand.class);
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    logger.error("ServletException in CreateAirportInstanceCommand::execute", ex1);
                } catch (IOException ex1) {
                    logger.error("IOException in CreateAirportInstanceCommand::execute", ex1);
                }
            }
        } catch (SQLException ex) {
            logger.error("SQLException in CreateAirportInstanceCommand::execute", ex);
        }
        
        try {
            response.sendRedirect("destinations");
        } catch (IOException ex) {
            logger.error("IOException in CreateAirportInstanceCommand::execute", ex);
        }
    }
